...
public class GlideImageLoader implements ImageLoaderInterface {
    ...
    public void display(ImageView imageView, String url) {
        display(imageView, url, R.drawable.ic_photo_24dp, R.drawable.ic_photo_24dp);
    }


    @Override
    public void display(ImageView imageView, int resId) {
        display(imageView, resId, R.drawable.ic_photo_24dp, R.drawable.ic_photo_24dp);
    }
    ...
    public void display(ImageView imageView, String url, int loadingImg, int errorImg) {
        ...
        DrawableTypeRequest<String> request = Glide.with(imageView.getContext()).load(url);
        if (0 != loadingImg) {
            request.placeholder(loadingImg);
        }
        if (0 != errorImg) {
            request.error(loadingImg);
        }
        request.//设置缓存策略，默认不缓存原始图，只缓存结果图
